...
# data_file = 'BGS_ANY_NGC_clustering.dat.fits'
# rand_file = 'BGS_ANY_NGC_1_clustering.ran.fits'
subsample_frac_data, subsample_frac_rand = 1, 1
z_min, z_max = 0.5, 1.0
edge_start, edge_end, edge_num = 20, 200, 181
...
w_data = data['WEIGHT'] * data['WEIGHT_FKP']
w_rand = rand['WEIGHT'] * rand['WEIGHT_FKP']
...
plt.plot(bin_centers, xi_ls, label='ξ(s)')
plt.axvline(105, color='red', linestyle='--', label='BAO scale (~105 Mpc/h)')
plt.xlabel("Separation s [Mpc/h]")
plt.ylabel("Correlation ξ(s)")
plt.ylim(-0.1, 0.2)  # zoom in on realistic BAO scale
plt.grid(True)
plt.legend()
